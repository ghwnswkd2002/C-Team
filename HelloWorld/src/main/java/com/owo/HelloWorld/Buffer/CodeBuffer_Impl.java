...
public class CodeBuffer_Impl implements CodeBuffer{
    ...
    public ParamBean case_VariableDeclaration(int lineNumber,String line,HashMap<String,String> splitcode,CoreBuffer corebuffer) {
        ...
        for(ParameterType parameterType : ParameterType.values()) {
            ...
            if(!line.contains("*")&&dev[0].toUpperCase().equals(parameterType.toString())&&!line.contains("=")) {
            ...
            else if(!line.contains("*")&&dev[0].equals("char")) {
            ...
            //포인터인 경우
                ...
                if(ConLine.contains("=")&&ConLine.contains(parameterType.toString().toLowerCase())) {
                  ...
                  //  System.out.println("^^^^^^^^^^^^^^^^^^^^^^^"+linepp);
                    ...
                    param.setParamName(plus.replaceAll(";",""));
                    param.setParamType(lineppp[0]);
                    System.out.println(plus.replaceAll(";"    ,""));
    ...
    public int nameSearch(CoreBuffer corebuffer,String code) {
        String key[];
        System.out.println("네임서치다코노야로");
        System.out.println(code);
        if(code.contains("*")&&code.contains("&")) {
            key = code.split("=");
            System.out.println("키1뭔데"+key[1]);
            key[0]=key[1].replaceAll(" ","");
            key[0]=key[0].replaceAll(";","");
            key[0]=key[0].replaceAll("&","");      //지워줘라

            System.out.println("키0뭔데"+key[0]);
        }
        else {
            key = code.split("=");
