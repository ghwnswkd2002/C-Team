...
var ColorEditor = function() {
	 ...
	 * 실시간 에디터를 비활성화한다.
	 */
	_object.disableRealTime = function(forceUpdate) {
		if (!realTimeEnabled && !forceUpdate) return;
...
ColorEditor.register = function (domObj, language, stylePackage) {
	if (!domObj) {
		throw Error("domObj is null");
		return null;
	}

	var ce = new ColorEditor();
	ce.init(domObj, language, stylePackage);
